 ...
 */
class Innoexts_WarehousePlus_Model_Catalog_Product_Attribute_Backend_Tierprice 
    extends Mage_Catalog_Model_Product_Attribute_Backend_Tierprice 
{
    ...
    public function validate($object)
    {
        ...
        $baseCurrency = $helper->getCurrencyHelper()->getBaseCode();
     ...
     * @return array
     */
    public function preparePriceData2(array $priceData, $productTypeId, $websiteId, $storeId, $currency, $stockId)
    {
        $helper                 = $this->getWarehouseHelper();
        $priceHelper            = $this->getProductPriceHelper();
        ...
        foreach ($priceData as $v) {
            ...
            if (
                !isset($data[$key]) && (
                    ...
                    (
                        ($stockId && (($v['stock_id'] == $stockId) || (!$v['stock_id']))) || 
                        (!$stockId && !$v['stock_id'])
                    )
                )
            ) {
