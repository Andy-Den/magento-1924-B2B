 ...
 */
class Innoexts_WarehousePlus_Model_Mysql4_Catalog_Product_Indexer_Price 
    extends Innoexts_Warehouse_Model_Mysql4_Catalog_Product_Indexer_Price 
{
    /**
    ...
    protected function __prepareBatchPriceIndex($entityIds = null, $attributeCode, $table, $indexTable)
    {
        ...
        $select->joinLeft(array('cbgp' => $table), implode(' AND ', array(
            '(cbgp.product_id = e.entity_id)', 
            '(cbgp.stock_id = cis.stock_id)', 
            '(cbgp.currency = cr.currency_to)', 
            '(cbgp.store_id = 0)', 
        )), array());
        if (!$priceHelper->isGlobalScope()) {
            if ($priceHelper->isWebsiteScope()) {
                $select->joinLeft(array('cbp' => $table), implode(' AND ', array(
                    '(cbp.product_id = e.entity_id)', 
                    '(cbp.stock_id = cis.stock_id)', 
                    '(cbp.currency = cr.currency_to)', 
                    '(csg.group_id = cw.default_group_id) AND (cbp.store_id = csg.default_store_id)', 
                )), array());
            } else {
                $select->joinLeft(array('cbp' => $table), implode(' AND ', array(
                    '(cbp.product_id = e.entity_id)', 
        ...
        if (!$priceHelper->isGlobalScope()) {
            $price = new Zend_Db_Expr("IF (
                ...
                ROUND(cbp.price / {$rate}, 8), 
                IF (
                    cbgp.price IS NOT NULL, 
                    ROUND(ROUND(cbgp.price * cwd.rate, 8) / {$rate}, 8), 
                    {$price}
                )
            )");
        } else {
    ...
    protected function _prepareBatchPriceIndex($entityIds = null)
    {
        return $this->__prepareBatchPriceIndex(
            ...
            $this->getProductPriceIndexerHelper()->getBatchPriceIndexTable()
        );
    }
    /**
    ...
    protected function _prepareBatchSpecialPriceIndex($entityIds = null)
    {
        return $this->__prepareBatchPriceIndex(
            ...
            $this->getProductPriceIndexerHelper()->getBatchSpecialPriceIndexTable()
        );
    }
    /**
