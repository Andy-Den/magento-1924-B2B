 ...
 */
class Innoexts_WarehousePlus_Model_Mysql4_Bundle_Indexer_Price 
    extends Innoexts_Warehouse_Model_Mysql4_Bundle_Indexer_Price 
{
    /**
    ...
    protected function _calculateBundleSelectionPrice($priceType)
    {
        ...
        $select = $write->select()
            ...
            ->join(array('idx' => $this->getIdxTable()), 
                ...
                '(i.stock_id = idx.stock_id) AND (i.currency = idx.currency) AND (i.store_id = idx.store_id)', array())
    ...
    protected function _calculateBundleOptionPrice()
    {
        ...
        if ($this->getVersionHelper()->isGe1600()) {
            $select = $write->select()
                ...
                ->group(array('entity_id', 'customer_group_id', 'website_id', 'option_id', 'stock_id', 'currency', 'store_id', 'is_required', 'group_type'));
            ...
            $select = $write->select()
                ->from(array('i' => $this->_getBundleSelectionTable()),
                    array('entity_id', 'customer_group_id', 'website_id', 'option_id'))
                ->group(array('entity_id', 'customer_group_id', 'website_id', 'option_id', 'stock_id', 'currency', 'store_id'))
                ->columns(array(
        ...
        if ($this->getVersionHelper()->isGe1600()) {
            ...
            $select = $write->select()
                ...
                ->join(
                    ...
                    '(i.stock_id = io.stock_id) AND (i.currency = io.currency) AND (i.store_id = io.store_id)',
                    array()
                )
                ->group(array(
                    'io.entity_id', 'io.customer_group_id', 'io.website_id', 
                    'io.stock_id', 'io.currency', 'io.store_id', 'i.tax_class_id', 'i.orig_price', 'i.price'
                ));
