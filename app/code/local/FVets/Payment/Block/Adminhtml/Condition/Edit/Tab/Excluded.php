 ...
 */
/**
 * condition - categories exclude relation edit block
 *
 * @category    FVets
 * @package     FVets_Payment
 * @author      Douglas Borella Ianitsky
 */
class FVets_Payment_Block_Adminhtml_Condition_Edit_Tab_Excluded extends Mage_Adminhtml_Block_Catalog_Category_Tree
{
    ...
    {
        parent::__construct();
        $this->setTemplate('fvets_payment/condition/edit/tab/excluded.phtml');
        $this->_withProductCount = false;
    }

    /**
     * Retrieve currently edited condition
     *
     * @access public
    ...
    /**
     * Return array with categories IDs which the condition is linked to
     *
     * @access public
     * @return array
     * @author Douglas Borella Ianitsky
    ...
    public function getCategoryIds()
    {
        if (is_null($this->_categoryIds)) {
            $categories = $this->getCondition()->getExcludedCategories();
            $ids = array();
     ...
     * Returns root node
     *
     * @param FVets_Payment_Model_Category|null $parentNodeCategory
    ...
    public function getRoot($parentNodeCategory = null, $recursionLevel = 3)
    {
        ...
        $root = Mage::registry('excluded_root');
        if (is_null($root)) {
            $rootId = Mage_Catalog_Model_Category::TREE_ROOT_ID;
            $ids = $this->getExcludedCategoryPathIds($rootId);
            $tree = Mage::getResourceSingleton('catalog/category_tree')
                ->loadByIds($ids, false, false);
            if ($this->getCategory()) {
                $tree->loadEnsuredNodes($this->getCategory(), $tree->getNodeById($rootId));
            }
            $tree->addCollectionData($this->getExcludedCategoryCollection());
            $root = $tree->getNodeById($rootId);
            Mage::register('excluded_root', $root);
        }
        return $root;
    }
    ...
    /**
     * Returns array with nodes those are selected (contain current condition)
     *
     * @return array
     */
    ...
    {
        return $this->getUrl('*/*/excludedJson', array('_current' => true));
    }
     ...
     * @author Douglas Borella Ianitsky
     */
    public function getExcludedCategoryPathIds($rootId = false)
    {
        $ids = array();
