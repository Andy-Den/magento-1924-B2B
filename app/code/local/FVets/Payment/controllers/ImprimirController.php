<?php
require_once(Mage::getModuleDir('controllers','Allpago_Gwap').DS.'ImprimirController.php');
class FVets_Payment_ImprimirController extends Allpago_Gwap_ImprimirController{
            ...
            if( $mOrder->getPayment()->getMethod() == 'gwap_boleto'){
                ...
                if( $gwapItem->hasData() && $gwapItem->getType() == 'boleto' ){
                    ...
                    if($gwapItem->getStatus() == Allpago_Gwap_Model_Order::STATUS_CREATED){
                        ...
                        $data = new Varien_Object(unserialize(Mage::helper('core')->decrypt($gwapItem->getInfo())));
                        if (is_array($data->getBoletoLink()))
                        {
                            foreach($data->getBoletoLink() as $link)
                            {
