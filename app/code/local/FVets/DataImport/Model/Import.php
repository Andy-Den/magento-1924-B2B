 ...
 * Import Main Model
 *
 * @method FVets_DataImport_Model_Import setBehavior(string $value)
 * @method string getBehavior()
 * @method FVets_DataImport_Model_Import setPartialIndexing(boolean $value)
 * @method boolean getPartialIndexing()
 * @method FVets_DataImport_Model_Import setContinueAfterErrors(boolean $value)
 * @method boolean getContinueAfterErrors()
 * @method FVets_DataImport_Model_Import setErrorLimit(boolean $value)
 * @method boolean getErrorLimit()
 * @method FVets_DataImport_Model_Import setUseNestedArrays(boolean $value)
 * @method boolean getUseNestedArrays()
 * @method FVets_DataImport_Model_Import setIgnoreDuplicates(boolean $value)
 * @method boolean getIgnoreDuplicates()
 * @method FVets_DataImport_Model_Import setAllowRenameFiles(boolean $value)
 * @method boolean getAllowRenameFiles()
 * @method FVets_DataImport_Model_Import setDisablePreprocessImageData(boolean $value)
 * @method boolean getDisablePreprocessImageData()
 * @method FVets_DataImport_Model_Import setUnsetEmptyFields(bool $value)
 * @method string getUnsetEmptyFields()
 * @method FVets_DataImport_Model_Import setSymbolEmptyFields(string $value)
 * @method string getSymbolEmptyFields()
 */
class FVets_DataImport_Model_Import extends Mage_ImportExport_Model_Import
{
    protected function _construct()
    {
        ...
        $this->setBehavior(Mage::getStoreConfig('fvetsdataimport/general/import_behavior'));
        $this->setPartialIndexing(Mage::getStoreConfigFlag('fvetsdataimport/general/partial_indexing'));
        $this->setContinueAfterErrors(Mage::getStoreConfigFlag('fvetsdataimport/general/continue_after_errors'));
        $this->setErrorLimit(intval(Mage::getStoreConfig('fvetsdataimport/general/error_limit')));
        $this->setUseNestedArrays(Mage::getStoreConfigFlag('fvetsdataimport/general/support_nested_arrays'));
        $this->setIgnoreDuplicates(Mage::getStoreConfigFlag('fvetsdataimport/general/ignore_duplicates'));
        $this->setDropdownAttributes(array_filter(explode(',', Mage::getStoreConfig('fvetsdataimport/product/select_attributes'))));
        $this->setMultiselectAttributes(array_filter(explode(',', Mage::getStoreConfig('fvetsdataimport/product/multiselect_attributes'))));
        $this->setAllowRenameFiles(Mage::getStoreConfigFlag('fvetsdataimport/product/allow_rename_files'));
        $this->setImageAttributes(array_filter(explode(',', Mage::getStoreConfig('fvetsdataimport/product/additional_image_attributes'))));
        $this->setDisablePreprocessImageData(Mage::getStoreConfigFlag('fvetsdataimport/product/clear_field_on_empty_string'));
        $this->setUnsetEmptyFields(! Mage::getStoreConfigFlag('fvetsdataimport/general/clear_field_on_empty_string'));
        $this->setSymbolEmptyFields(Mage::getStoreConfig('fvetsdataimport/general/symbol_for_clear_field'));
    }
     ...
     *
     * @return FVets_DataImport_Model_Import
     */
    public function processProductImport($data, $behavior = NULL)
    {
        $transport = new Varien_Object(array('import_data' => $data));
        Mage::dispatchEvent('fvetsdataimport_import_products_before', array('import_data' => $transport));
        ...
        /** @var $entityAdapter FVets_DataImport_Model_Import_Entity_Product */
        $entityAdapter = Mage::getModel('fvetsdataimport/import_entity_product');
    ...
    {
        $transport = new Varien_Object(array('import_data' => $data));
        Mage::dispatchEvent('fvetsdataimport_dryrun_products_before', array('import_data' => $transport));
        $data = $transport->getImportData();
    ...
    {
        $transport = new Varien_Object(array('import_data' => $data));
        Mage::dispatchEvent('fvetsdataimport_import_customers_before', array('import_data' => $transport));
        $data = $transport->getImportData();
        ...
        /** @var $entityAdapter FVets_DataImport_Model_Import_Entity_Customer */
        $entityAdapter = Mage::getModel('fvetsdataimport/import_entity_customer');
        $entityAdapter->setBehavior($this->getBehavior());
    ...
    {
        $transport = new Varien_Object(array('import_data' => $data));
        Mage::dispatchEvent('fvetsdataimport_dryrun_customers_before', array('import_data' => $transport));
        $data = $transport->getImportData();
    ...
    {
        $transport = new Varien_Object(array('import_data' => $data));
        Mage::dispatchEvent('fvetsdataimport_import_categories_before', array('import_data' => $transport));
        $data = $transport->getImportData();
        ...
        $partialIndexing = $this->getPartialIndexing();
        /** @var $entityAdapter FVets_DataImport_Model_Import_Entity_Category */
        $entityAdapter = Mage::getModel('fvetsdataimport/import_entity_category');
        $entityAdapter->setBehavior($this->getBehavior());
        $entityAdapter->setErrorLimit($this->getErrorLimit());
        $entityAdapter->setIgnoreDuplicates($this->getIgnoreDuplicates());
        $entityAdapter->setUnsetEmptyFields($this->getUnsetEmptyFields());
    ...
    {
        $transport = new Varien_Object(array('import_data' => $data));
        Mage::dispatchEvent('fvetsdataimport_dryrun_categories_before', array('import_data' => $transport));
        $data = $transport->getImportData();
    ...
    {
        $transport = new Varien_Object(array('import_data' => $data));
        Mage::dispatchEvent('fvetsdataimport_import_categoryproducts_before', array('import_data' => $transport));
        $data = $transport->getImportData();
        ...
        /** @var $entityAdapter FVets_DataImport_Model_Import_Entity_Category_Product */
        $entityAdapter = Mage::getModel('fvetsdataimport/import_entity_category_product');
        $entityAdapter->setBehavior($this->getBehavior());
        $entityAdapter->setErrorLimit($this->getErrorLimit());
        $entityAdapter->setIgnoreDuplicates($this->getIgnoreDuplicates());
//        $entityAdapter->setUnsetEmptyFields($this->getUnsetEmptyFields());
//        $entityAdapter->setSymbolEmptyFields($this->getSymbolEmptyFields());
    ...
    {
        $transport = new Varien_Object(array('import_data' => $data));
        Mage::dispatchEvent('fvetsdataimport_dryrun_categoryproducts_before', array('import_data' => $transport));
        $data = $transport->getImportData();
     ...
     *
     * @return FVets_DataImport_Model_ArrayAdapter
     */
    protected function _getSourceAdapter($sourceData)
    {
        if (is_array($sourceData)) {
            if ($this->getUseNestedArrays()) {
                return Mage::getModel('fvetsdataimport/nestedArrayAdapter', $sourceData);
            } else {
                return Mage::getModel('fvetsdataimport/arrayAdapter', $sourceData);
            }
        }
