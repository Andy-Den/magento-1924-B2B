...
class FVets_CustomGrid_Block_Widget_Grid_Columns_Config
    extends Mage_Adminhtml_Block_Widget
{
    ...
    protected function _construct()
    {
        ...
        $this->setTemplate('fvets/customgrid/widget/grid/columns/config.phtml');
    }
    ...
    {
        return parent::getButtonHtml($this->__('Grid Customization'), '$(\''.$this->getHtmlId().'\').toggle()', 'scalable fvetscg-customize');
    }
    ...
    {
        return parent::getButtonHtml($this->__('Grid Infos'), '$(\''.$this->getHtmlId().'-grid-infos\').toggle();', 'scalable fvetscg-grid-infos');
    }
    
    public function getToggleAdditionalButtonHtml()
    {
        return parent::getButtonHtml($this->__('More Options'), '$(\''.$this->getHtmlId().'-additional\').toggle();', 'scalable fvetscg-additional');
    }
    ...
    public function getGridFilterParamValue()
    {
        if (($grid = $this->getGridBlock()) && !$grid->getUseAjax()
            ...
            && ($param = $grid->fvetscg_getFilterParam())) {
            return $param;
        }
        return null;
    }
    ...
    public function canDisplayColumnsConfig()
    {
        if (!$this->hasData('can_display_columns_config')) {
            $this->setData(
                ...
                $this->getGridModel()
                    ->checkUserActionPermission(FVets_CustomGrid_Model_Grid::GRID_ACTION_CUSTOMIZE_COLUMNS)
            );
        }
            ...
            $this->setData(
                ...
                ($this->getGridBlock()
                 ...
                 && (($this->getGridModel()->canHaveCustomColumns() && $this->canDisplayColumnsConfig())
                     || $this->getGridModel()->checkUserActionPermission(FVets_CustomGrid_Model_Grid::GRID_ACTION_EDIT_DEFAULT_PARAMS)
        ...
        if (!$this->hasData('can_display_grid_infos')) {
            $this->setData(
                ...
                (!$this->getIsNewModel()
                 && $this->getGridBlock()
                 && $this->getGridModel()
                        ->checkUserActionPermission(FVets_CustomGrid_Model_Grid::GRID_ACTION_VIEW_GRID_INFOS))
            );
        }
