...
class FVets_CustomGrid_Block_Widget_Grid_Columns_Config_Columns
    extends FVets_CustomGrid_Block_Widget_Grid_Columns_Config_Abstract
{
    ...
    {
        parent::_construct();
        $this->setTemplate('fvets/customgrid/widget/grid/columns/config/columns.phtml');
    }
    ...
    public function canChooseEditableColumns()
    {
        if (!$this->hasData('can_choose_editable_columns')) {
            $this->setData(
                ...
                $this->getGridModel()->checkUserActionPermission(FVets_CustomGrid_Model_Grid::GRID_ACTION_CHOOSE_EDITABLE_COLUMNS)
            );
        }
