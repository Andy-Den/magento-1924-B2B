...
class FVets_CustomGrid_Block_Widget_Grid_Column_Renderer_Order_Status_Color
    ...
    protected function _renderRow(Varien_Object $row, $forExport=false)
    {
        ...
        if (!empty($options) && is_array($options)) {
            ...
            if (!$forExport && isset($colors[$value])) {
                $elementId = Mage::helper('core')->uniqHash('fvetscg-gcr-osc-');
                $onlyCell  = ((bool)$this->getColumn()->getOnlyCell() ? 'true' : 'false');
                
                $return   .= '<span id="'.$elementId.'"></span>'
                    . '<script type="text/javascript">'
                    . 'fvetscg.CustomColumn.OptionsColor.registerRowChange("'.$elementId.'", "'
