...
class FVets_CustomGrid_Custom_GridController
    extends Mage_Adminhtml_Controller_Action
{
            ...
            if (($grid = $this->_initGrid()) && $grid->getId()) {
                try {
                    ...
                    if (isset($data['config'])
                        && $grid->checkUserActionPermission(FVets_CustomGrid_Model_Grid::GRID_ACTION_CUSTOMIZE_COLUMNS)) {
                        // Columns config
    ...
    public function saveDefaultAction()
    {
        if ($this->getRequest()->isPost()) {
            ...
            if (($grid = $this->_initGrid()) && $grid->getId()
                && $grid->checkUserActionPermission(FVets_CustomGrid_Model_Grid::GRID_ACTION_EDIT_DEFAULT_PARAMS)) {
                try {
    ...
    public function exportCsvAction()
    {
        ...
        if (($grid = $this->_initGrid()) && $grid->getId()
            && $grid->checkUserActionPermission(FVets_CustomGrid_Model_Grid::GRID_ACTION_EXPORT_RESULTS)) {
            try {
                $fileName = 'export.csv';
