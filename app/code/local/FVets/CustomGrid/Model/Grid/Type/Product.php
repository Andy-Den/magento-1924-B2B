...
class FVets_CustomGrid_Model_Grid_Type_Product
    extends FVets_CustomGrid_Model_Grid_Type_Abstract
{
    ...
    {
        return array('store_id' => $grid->fvetscg_getStore()->getId());
    }
    ...
    protected function _applyEditedFieldValue($type, $config, $params, $entity, $value)
    {
        if ($config['id'] == 'qty') {
            ...
            $entity->setData('_fvetscg_gtp_stock_item', $stockItem);
            return $this;
        }
    ...
    {
        if ($config['id'] == 'qty') {
            if ($stockItem = $entity->getData('_fvetscg_gtp_stock_item')) {
                $stockItem->save();
            }
            return $this;
        }
    ...
    public function afterGridSetCollection($grid, $collection)
    {
        if ($this->_mustCaptureExportedCollection) {
            ...
            $grid->fvetscg_setExportedCollection($clone);
        }
        return $this;
    }
