...
class FVets_CustomGrid_Model_Grid extends Mage_Core_Model_Abstract
{
    /**
    * Session params keys
    */
    const SESSION_BASE_KEY_CURRENT_PROFILE = '_fvetscg_session_key_current_profile_';
    const SESSION_BASE_KEY_APPLIED_FILTERS = '_fvetscg_session_key_applied_filters_';
    const SESSION_BASE_KEY_REMOVED_FILTERS = '_fvetscg_session_key_removed_filters_';
    const SESSION_BASE_KEY_TOKEN = '_fvetscg_session_key_token_';
    /**
    * Parameter name to use to hold grid token value
    */
    const GRID_TOKEN_PARAM_NAME  = '_fvetscg_token_';
    
    /**
    * Attribute columns base keys
    */
    const GRID_COLUMN_ATTRIBUTE_ID_PREFIX  = '_fvetscg_attribute_column_';
    const GRID_COLUMN_ATTRIBUTE_GRID_ALIAS = 'fvetscg_attribute_field_';
    
    /**
    * Custom columns base keys
    */
    const GRID_COLUMN_CUSTOM_ID_PREFIX  = '_fvetscg_custom_column_';
    const GRID_COLUMN_CUSTOM_GRID_ALIAS = 'fvetscg_custom_field_';
    ...
    * 
    * @return FVets_CustomGrid_Model_Grid_Type_Abstract
    */
    ...
    */
    protected function _exportFile($format, $infos=null)
    {
        ...
        if (!is_null($infos)) {
            $block->fvetscg_setExportInfos($infos);
        }
        if (!is_null($this->_typeModel)) {
    ...
    * 
    * @param string $index Column index
    * @param FVets_CustomGrid_Model_Custom_Column_Abstract $customColumn Custom column model
    * @param string $rendererType Renderer type code
    ...
    */
    public function verifyGridBlockFilters(Mage_Adminhtml_Block_Widget_Grid $grid, $filters)
    {
        ...
        */
        $tokenValue = Mage::helper('core')->uniqHash('fvetscg:');
        ...
        */
        ...
        if ($grid->fvetscg_getSaveParametersInSession()) {
            $session->setData($grid->fvetscg_getSessionParamKey($grid->getVarNameFilter()), $filterParam);
        }
        if ($grid->getRequest()->has($grid->getVarNameFilter())) {
            $grid->getRequest()->setParam($grid->getVarNameFilter(), $filterParam);
        }
        $grid->fvetscg_setFilterParam($filterParam);
    ...
    */
    public function applyBaseDefaultLimitToGridBlock(Mage_Adminhtml_Block_Widget_Grid $grid)
    {
        ...
        $grid->fvetscg_setDefaultLimit($defaultLimit, true);
        return $this;
    }
    ...
    public function applyDefaultToGridBlock(Mage_Adminhtml_Block_Widget_Grid $grid)
    {
        ...
        if ($filters = $this->_getData('default_filter')) {
            if (is_array($filters = @unserialize($filters))) {
                ...
                $grid->fvetscg_setDefaultFilter($appliable);
            }
        }
        return $this;
    }
    ...
    */
    public function applyColumnsToGridBlock(Mage_Adminhtml_Block_Widget_Grid $grid, $applyFromCollection)
    {
            ...
            if (!in_array($column['id'], $gridIds, true)) {
                if ($column['is_visible'] && !$column['missing']
                    ...
                    if ($this->isCollectionColumnOrigin($column['origin'])) {
                        ...
                        $grid->fvetscg_addAdditionalAttribute(array(
                            'alias'     => $alias,
                ...
                if ($column['is_visible']) {
                    ...
                    $grid->fvetscg_removeColumn($column['id']);
                }
            }
        ...
        $grid->fvetscg_resetColumnsOrder();
        $previousId = null;
