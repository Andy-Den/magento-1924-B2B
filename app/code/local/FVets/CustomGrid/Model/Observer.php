...
class FVets_CustomGrid_Model_Observer
{
    ...
    *
    * @var FVets_CustomGrid_Model_Mysql4_Grid_Collection
    */
    ...
    *
    * @param FVets_CustomGrid_Model_Grid $grid
    * @return bool
    */
        ...
        if ((!$rewritingClassName && ($grid->getRewritingClassName() == ''))
            ...
            if ($this->_getConfigHelper()->isExcludedGrid($grid->getBlockType(), $rewritingClassName)) {
                ...
                foreach ($rewriters as $rewriter) {
                    try {
                        $fvetscgClass = $rewriter->rewriteGrid($originalClass, $grid->getBlockType());
                    } catch (Exception $e) {
                        $fvetscgClass = false;
    ...
    *
    * @return FVets_CustomGrid_Model_Grid
    */
    ...
    public function beforeControllerActionLayoutLoad($observer)
    {
        if ($layout = $observer->getLayout()) {
            ...
            if ($this->_getHelper()->isMageVersionLesserThan(1, 7)) {
                $layout->getUpdate()->addHandle('fvetscg_magento_version_to_16');
            } else {
                $layout->getUpdate()->addHandle('fvetscg_magento_version_from_17');
            }
        }
    }
            ...
            if ($grid->getTemplate() == 'widget/grid.phtml') {
                ...
                if (!$model->getDisabled()
                    ...
                    if ($helper->isMageVersionGreaterThan(1, 5)) {
                        $grid->setTemplate('fvets/customgrid/widget/grid/16.phtml');
                    } elseif ($helper->isMageVersion15()) {
                        $grid->setTemplate('fvets/customgrid/widget/grid/15.phtml');
                    } else {
                        $revision = $helper->getMageVersionRevision();
                        $grid->setTemplate('fvets/customgrid/widget/grid/14'.intval($revision).'.phtml');
                    }
                }
    ...
    */
            ...
            && ($grid instanceof Mage_Adminhtml_Block_Widget_Grid)) {
                ...
                if (($model = $this->_getGridModel($blockType, $blockId, true))
                    ...
                    if ($this->_getHelper()->isRewritedGrid($grid)) {
                        ...
                        $grid->fvetscg_setGridModel($model)->fvetscg_setTypeModel($model->getTypeModel());
                    } else {
    ...
    public function beforeGridPrepareCollection($grid)
    {
        ...
        if (!is_null($model = $this->_getGridModel($blockType, $blockId, true))
            ...
            if ($model->checkUserActionPermission(FVets_CustomGrid_Model_Grid::GRID_ACTION_USE_DEFAULT_PARAMS)) {
            ...
            $grid->fvetscg_holdPrepareCollection();
        }
    }
    ...
    public function afterGridPrepareCollection(Mage_Adminhtml_Block_Widget_Grid $grid)
    {
        ...
        if (!is_null($model = $this->_getGridModel($blockType, $blockId, true))
            ...
            if ($grid->getCollection()) {
                ...
                if ($model->checkUserActionPermission(FVets_CustomGrid_Model_Grid::GRID_ACTION_USE_CUSTOMIZED_COLUMNS)) {
                ...
                $grid->fvetscg_finishPrepareCollection();
            } else {
