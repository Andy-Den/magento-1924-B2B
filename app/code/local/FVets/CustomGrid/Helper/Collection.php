...
class FVets_CustomGrid_Helper_Collection
    extends Mage_Core_Helper_Abstract
{
    const COLLECTION_APPLIED_MAP_FLAG   = '_fvetscg_hc_applied_map_';
    const COLLECTION_PREVIOUS_MAP_FLAG  = '_fvetscg_hc_previous_map_';
        ...
        foreach ($block->getColumns() as $columnId => $column) {
            if (isset($filters[$columnId])
                ...
                if ((strpos($field, '.') === false) // @todo not completely safe as "." is allowed in quoted identifier
                    ...
                    && (strpos($field, FVets_CustomGrid_Model_Grid::GRID_COLUMN_ATTRIBUTE_GRID_ALIAS) !== 0)
                    && (strpos($field, FVets_CustomGrid_Model_Grid::GRID_COLUMN_CUSTOM_GRID_ALIAS) !== 0)) {
