...
class FVets_SalesRule_Model_Mix_Layer extends Mage_Catalog_Model_Layer
{
    ...
    private function getRuleConditionProduct($rule)
    {
        if (isset($rule['conditions']))
        {
            foreach ($rule['conditions'] as $condition)
            {
                ...
                {
                    return $this->getRuleConditionProduct($condition);
                }
            }
        }
    ...
    {
        $unserialized = unserialize(Mage::registry('current_rule')->getConditionsSerialized());

        $condition = $this->getRuleConditionProduct($unserialized);
        ...
        {
            $condition = array('attribute' => 'id_erp', 'value' => array(0));
        }

        $collection
            ->addAttributeToSelect(Mage::getSingleton('catalog/config')->getProductAttributes())
            ->addAttributeToFilter($condition['attribute'], array('in' => $condition['value']))
