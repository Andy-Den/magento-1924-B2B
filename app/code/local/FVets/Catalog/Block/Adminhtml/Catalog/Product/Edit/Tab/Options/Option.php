...
class FVets_Catalog_Block_Adminhtml_Catalog_Product_Edit_Tab_Options_Option extends Mage_Adminhtml_Block_Catalog_Product_Edit_Tab_Options_Option
{
    public function getWebsiteSelectHtml()
    {
        $select = $this->getLayout()->createBlock('adminhtml/html_select')
            ->setData(array(
                'id' => $this->getFieldId().'_{{id}}_website_id',
                'class' => 'select'
            ))
            ->setName($this->getFieldName().'[{{id}}][website_id]')
            ->setOptions(Mage::getSingleton('core/resource_website_collection')->toOptionArray());
...
//        $optionsArr = $this->getProduct()->getOptions();
        ...
        if (!$this->_values) {
            ...
            foreach ($optionsArr as $option) {
                ...
                $value['website_id'] = $option->getWebsiteId();
...
//                    $valuesArr = array_reverse($option->getValues(), true);
                    ...
                    foreach ($option->getValues() as $_value) {
                        ...
                        $value['optionValues'][$i] = array(
                            ...
                            'action' => $_value->getAction(),
                            'sku' => $this->escapeHtml($_value->getSku()),
                            'multiply_factor' => $_value->getMultiplyFactor(),
                            'id_erp' => $_value->getIdErp(),
                            'sort_order' => $_value->getSortOrder(),
                            'website_id' => $_value->getWebsiteId(),
                        );
                    ...
                    $value['action'] = $option->getAction();
                    $value['multiply_factor'] = $option->getMultiplyFactor();
                    $value['id_erp'] = $option->getIdErp();
