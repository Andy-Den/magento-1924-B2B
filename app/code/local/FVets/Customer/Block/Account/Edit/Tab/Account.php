...
class FVets_Customer_Block_Account_Edit_Tab_Account extends Mage_Adminhtml_Block_Customer_Edit_Tab_Account
{
...
//        }
        ...
        if ($suffixElement) {
			...
			//adicionando checkbox para envio de email de ativação de cliente
			$fieldset->addField('send_activate_email', 'checkbox', array(
				'label' => Mage::helper('customer')->__('Send activate account email?'),
				'onclick' => 'this.value = this.checked ? 1 : 0;',
				'name' => 'send_activate_email',
			));
			$form->getElement('send_activate_email')->setIsChecked(!empty($formData['send_activate_email']));

			$fieldset->addField('is_active', 'select',
				array(
					'name'  => 'is_active',
					'label' => Mage::helper('customer')->__('Status'),
					'title' => Mage::helper('customer')->__('Status'),
					'values' => array(0 => 'Inativo', 1=>'Ativo')
				)
			);
			$form->getElement('is_active')->setValue(!empty($formData['is_active']));

        if ($customer->getId()) {
            if (!$customer->isReadonly() || Mage::getSingleton('admin/session')->isAllowed('customer/password_editable')) {
        ...
        if ($customer->isReadonly()) {
			$emailEditable = Mage::getSingleton('admin/session')->isAllowed('customer/email_editable');
            foreach ($customer->getAttributes() as $attribute) {
                $element = $form->getElement($attribute->getAttributeCode());

				if ($attribute->getAttributeCode() == 'email' && $emailEditable) {
					continue;
				}

				if (!isset($customer->getData()[$attribute->getAttributeCode()])) {
					continue;
				}
