...
class FVets_CatalogInventory_Model_Stock_Item extends Innoexts_Warehouse_Model_Cataloginventory_Stock_Item
{
	public function checkQuoteItemQty($qty, $summaryQty, $origQty = 0)
	{
		...
		if (!$this->checkQty($summaryQty) || !$this->checkQty($qty)) {
			$message = Mage::helper('cataloginventory')->__('Apenas %s unidade(s) deste produto estão disponíveis', number_format($this->getQty(), 0, ",", "."));
			$result->setHasError(true)
				->setMessage($message)
				->setQuoteMessage($message)
				->setQuoteMessageIndex('qty')
				->setItemUseOldQty($this->getQty());
			return $result;
		} else {
