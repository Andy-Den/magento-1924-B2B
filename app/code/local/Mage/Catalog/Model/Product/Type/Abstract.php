...
//                    $buyRequest->setQty($buyRequest->getQty() + $qty * $option->getValues()[$confItemOption->getValue()]->getMultiplyFactor());
//                }
//            }
//            if ($buyRequest->getQty() == 0)
//                $buyRequest->getQty($qty);
//        }
    ...
    public function hasRequiredOptions($product = null)
    {
		...
		if(!empty($_options)) {
			foreach($_options as $_option) {
				if ($_option->getWebsiteId() == $currentWebsite) {
					return true;
				}
			}
			return false;
