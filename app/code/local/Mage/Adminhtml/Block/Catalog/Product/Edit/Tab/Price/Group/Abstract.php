    ...
    implements Varien_Data_Form_Element_Renderer_Interface
{
    ...
    public function getValues()
    {
        ...
        foreach ($values as &$value) {
            ...
            Mage::helper('amrolepermissions')->setSkipRestrition(true);
            $group = Mage::getModel('customer/group')->load($value['cust_group']);
            Mage::helper('amrolepermissions')->setSkipRestrition(false);

            if ($group->getWebsiteId() != 0 && is_array($rule->getScopeWebsites()) && !in_array($group->getWebsiteId(), $rule->getScopeWebsites()))
            {
                $value['hidden'] = true;
            } else {
                $value['hidden'] = false;
    ...
    public function getCustomerGroups($groupId = null)
    {
        if ($this->_customerGroups === null) {
            ...
            $collection = Mage::getModel('customer/group')->getCollection()->addWebsiteFilter();
