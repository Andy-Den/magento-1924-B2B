	...
	protected function _prepareResponse($data = array())
	{
		...
		if ($response->isRedirect()) {
			...
			if (!$data['status']) {
				...
				//adicionando ao response, a mensagem de erro
				if (isset($messageBlock) && isset($messageBlock->getMessages()[0])) {
					$errorMsg = $messageBlock->getMessages()[0]->getCode();
					if (isset($errorMsg)) {
...
//            }
		...
		$data['cartItemsCount'] = $cartItemsCount;
...
//        $layout->generateBlocks();
	...
	public function refreshToolbarVars(Varien_Event_Observer $observer)
	{
		...
		$supportedModules = array('catalog', 'catalogsearch', 'attributepages', 'attributemenu');
