...
class TM_ProLabels_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getLabel_(Mage_Catalog_Model_Product $product, $mode = 'product')
    {
        ...
        foreach ($labelsData as $data) {
            ...
            $html .= $data[$mode . '_position_style'].'"
                    class = "prolabel '
                    . $data[$mode . '_position'] . '">
                <span ' . (!empty($data['product_alt_text']) ? ('title="' . $data['product_alt_text'] . '"') : '') . ' class="prolabels-image" ' . $onClick . ' style="background: url(' . Mage::getBaseUrl('media') .
                     'prolabel/' .
                     $labelImg .
	...
	{
		$_block = Mage::app()->getLayout()->createBlock('prolabels/fvetslabel');
		return $_block->setProduct($product)->setMode($mode)->_toHtml();
	}

	//#julio 03/11/2014
	public function getLabelImgUrl(Mage_Catalog_Model_Product $product, $mode = 'product')
	{
		if (!Mage::getStoreConfig("prolabels/general/enabled")){
			return;
		}
	...
	public function getAltText(Mage_Catalog_Model_Product $product, $mode = 'product')
	{
		if (!Mage::getStoreConfig("prolabels/general/enabled")){
			return;
		}
	...
	public function getLabelText(Mage_Catalog_Model_Product $product, $mode = 'product')
	{
		if (!Mage::getStoreConfig("prolabels/general/enabled")){
			return;
		}
...
//        $this->_isOnSale($object, $mode, $label);
        foreach (current($vars) as $var) {
            ...
            if ($var == '#discount_amount#') {
                if ($object->getData('type_id') === 'bundle') {
                    ...
                    $priceArray = array();
                    if ($rulePrice = Mage::getModel('catalogrule/rule')->calcProductPriceRule($object, $object->getData('price'))) {
                        $priceArray[] = $object->getData('price') - $rulePrice;
                    } elseif($finalPrice = $object->getData('final_price')) {
                        $priceArray[] = $object->getData('price') - $finalPrice;
                    } elseif($specialPrice = $object->getData('special_price')) {
                        $priceArray[] = $object->getData('price') - $specialPrice;
                    }
                    $data[$var] = max($priceArray);
                }
            ...
            if ($var == '#discount_percent#') {
                ...
                if ($object->getData('type_id') === 'bundle') {
                    ...
                    $priceArray = array();
                    if ($rulePrice = Mage::getModel('catalogrule/rule')->calcProductPriceRule($object, $object->getData('price'))) {
                        $priceArray[] = (100 - $rulePrice * 100 / $object->getData('price')) / $label[$mode . '_round'];
                    } elseif($finalPrice = $object->getData('final_price')) {
                        $priceArray[] = (100 - $finalPrice * 100 / $object->getData('price')) / $label[$mode . '_round'];
                    } elseif($specialPrice = $object->getData('special_price')) {
                        $priceArray[] = (100 - $specialPrice * 100 / $object->getData('price')) / $label[$mode . '_round'];
                    }
                    $data[$var] = max($priceArray);
                }
    ...
    public function _isOnSale($product, $mode, $data)
    {
        ...
        if ($product->getTypeInstance() instanceof Mage_Bundle_Model_Product_Type) {
            ...
            $priceArray = array();
            if ($rulePrice = Mage::getModel('catalogrule/rule')->calcProductPriceRule($object, $object->getData('price'))) {
                $priceArray[] = (100 - $rulePrice * 100 / $object->getData('price')) / 1;
            } elseif($finalPrice = $object->getData('final_price')) {
                $priceArray[] = (100 - $finalPrice * 100 / $object->getData('price')) / 1;
            } elseif($specialPrice = $object->getData('special_price')) {
                $priceArray[] = (100 - $specialPrice * 100 / $object->getData('price')) / 1;
            }
            $sale = max($priceArray);
            if ($sale > 0) {
                return true;
