    ...
    implements Mage_Widget_Block_Interface
{
    ...
    public function getCacheKeyInfo()
    {
        ...
        $restriction_groups = Mage::helper('fvets_catalogrestrictiongroup/customer')->getSelectedCatalogrestrictiongroups($customer);
        foreach($restriction_groups as $key => $group) {
            $restriction_groups[$key] = $group->getId();
        }
        ...
        return array(
           ...
           $this->getNameInLayout(),
            $customer->getFvetsSalesrep(),
            $customer->getRestrictedBrands(),
            implode(',', $restriction_groups)
        );
    }
...
//            Mage::getSingleton('catalog/product_visibility')->addVisibleInCatalogFilterToCollection($collection);
	...
	{
		//verifica se o atributo para adicionar produtos randômicos caso não hajam produtos suficientes na collection;
		$collection = $this->getCollection();
		$_collectionSize = $collection->getSize();
		$flagRandomItems = $this->getRandomItems();
		$_productsCount = $this->getProductsCount();

		if (isset($flagRandomItems) && ($flagRandomItems == 1) && ($_collectionSize < $_productsCount)) {
				//geração da lista de produtos randômicos
				$_randomProductCollection = Mage::getResourceModel('catalog/product_collection');
				Mage::getModel('catalog/layer')->prepareProductCollection($_randomProductCollection);
				$_randomProductCollection->getSelect()->order('rand()');
				$_randomProductCollection->addStoreFilter();
				//fim da geração
				$_randomCollectionSize = $_randomProductCollection->count();
				$collectionIds = $collection->getAllIds();
				if ($_randomCollectionSize) {
					foreach ($_randomProductCollection as $_randomItem) {
						if (!in_array($_randomItem->getId(), $collectionIds)) {
							$collection->addItem($_randomItem);
						}
						if ($collection->count() == $_productsCount) {
							break;
						}
					}
				}
