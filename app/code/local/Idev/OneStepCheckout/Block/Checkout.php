...
class Idev_OneStepCheckout_Block_Checkout extends Mage_Checkout_Block_Onepage_Abstract  {
    ...
    {
        if($this->hasShippingErrors() || $this->hasBillingErrors() || $this->hasMethodErrors() || $this->hasShipmentErrors() || $this->hasUnkownSourceErrors()) {
            return true;
        }
    ...
    public function hasUnkownSourceErrors()
    {
        if(isset($this->formErrors['unknown_source_error']))  {
            if(count($this->formErrors['unknown_source_error']) == 0) {
                return false;
            }
            return true;
        }
    ...
    protected function _saveOrder()
    {
        ...
        try {
            ...
            Mage::logException($e);
            Mage::helper('onestepcheckout/checkout')->sendOrderFailedEmail($this->getOnepage()->getQuote(), $error);
            return;
            //die('Error: ' . $e->getMessage());
        }
    ...
    public function getBillingFieldsOrder($fields = array()){
        ...
        $fieldsAvailable = array(
			...
			'telephone' => array(),
			'mobilephone' => array(),
			//'address' => array('fields' => array('street', 'bairro', 'city', 'region_id', 'postcode')),
            'street' => array(),
            'country_id' => array(),
			'postcode' => array(),
			'region_id' => array(),
