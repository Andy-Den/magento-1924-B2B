...
class Idev_OneStepCheckout_Helper_Checkout extends Mage_Core_Helper_Abstract
{
    ...
    public function loadConfig()
    {
        ...
        $items = array(
            ...
            'general/enable_geoip', 'general/skin', 'exclude_fields/exclude_razao_social', 'exclude_fields/exclude_cnpj', 'exclude_fields/exclude_cpf', 'exclude_fields/exclude_inscricao_estadual', 'exclude_fields/exclude_incricao_estadual', 'exclude_fields/exclude_city', 'exclude_fields/exclude_bairro', 'exclude_fields/exclude_telephone', 'exclude_fields/exclude_mobilephone', 'general/show_custom_options',
            'exclude_fields/exclude_company', 'exclude_fields/exclude_fax', 'exclude_fields/exclude_region', 'exclude_fields/exclude_zip', 'exclude_fields/enable_comments', 'exclude_fields/enable_discount', 'exclude_fields/exclude_address', 'exclude_fields/exclude_saveaddress', 'exclude_fields/exclude_country_id', 'exclude_fields/enable_giftcard',
            ...
            'general/display_tax_included', 'general/update_address_and_customer', 'general/single_address', 'exclude_fields/newsletter_default_checked', 'feedback','addressreview',
            'general/display_full_tax','exclude_fields/enable_comments_default','terms/enable_default_terms','terms/enable_default_terms', 'terms/enable_textarea','general/checkout_logo'
        );
    ...
    public function saveInitialData($billing_data)
    {
        $billing_address = $this->getOnepage()->getQuote()->getBillingAddress();
				...
				$billing_address->setRazaoSocial($billing_data['razao_social']);
				$billing_address->setCnpj($billing_data['cnpj']);
				$billing_address->setCpf($billing_data['cpf']);
				$billing_address->setInscricaoEstadual($billing_data['inscricao_estadual']);
        ...
        $billing_address->setCity($billing_data['city']);
				$billing_address->setBairro($billing_data['bairro']);
        ...
        $billing_address->setTelephone($billing_data['telephone']);
				$billing_address->setMobilephone($billing_data['mobilephone']);
    ...
    public function load_exclude_data($data)
    {
        if( $this->settings['exclude_city']  || empty($data['city']))    {
            $data['city'] = '-';
				...
				if( $this->settings['exclude_bairro']  || empty($data['bairro']))    {
					$data['bairro'] = '-';
				}
        ...
        if( $this->settings['exclude_telephone'] || empty($data['telephone']))    {
            $data['telephone'] = '-';
				...
				if( $this->settings['exclude_mobilephone'] || empty($data['mobilephone']))    {
					$data['mobilephone'] = '-';
				}
    ...
    public function _getAddressError($result, $billing_data, $type = 'billing')
    {
        ...
        if(is_array($result['message']) && in_array('Please enter city.', $result['message']))    {
            $errors[] = 'city';
				...
				if(is_array($result['message']) && in_array('Please enter district.', $result['message']))    {
					$errors[] = 'bairro';
				}
	 ...
	 */
	public function sendOrderFailedEmail($checkout, $message, $checkoutType = 'onepage')
	{
		...
		$template = Mage::getStoreConfig('checkout/order_failed/template', $checkout->getStoreId());

		$copyTo = $this->_getEmails('checkout/order_failed/copy_to', $checkout->getStoreId());
		$copyMethod = Mage::getStoreConfig('checkout/order_failed/copy_method', $checkout->getStoreId());
		if ($copyTo && $copyMethod == 'bcc') {
			$mailTemplate->addBcc($copyTo);
		}

		$_reciever = Mage::getStoreConfig('checkout/order_failed/reciever', $checkout->getStoreId());
		$sendTo = array(
			array(
		...
		foreach ($sendTo as $recipient) {
			$mailTemplate->setDesignConfig(array('area'=>'frontend', 'store'=>$checkout->getStoreId()))
				->sendTransactional(
					$template,
					Mage::getStoreConfig('checkout/order_failed/identity', $checkout->getStoreId()),
					$recipient['email'],
					$recipient['name'],
					array(
						'reason' => $message,
