...
class Idev_OneStepCheckout_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    {
        parent::preDispatch();
		if (Mage::getStoreConfig('onestepcheckout/general/validate_index_customer')) {
    ...
    public function indexAction() {
		...
		if (!$quote->validateMinimumAmount()) {
			...
			Mage::getSingleton('checkout/cart')->getCheckoutSession()->addNotice($warning);
			$this->_redirect('checkout/cart');
			return;
		}
	...
	public function resetPasswordOnCheckoutPostAction()
	{
		...
		if (!Zend_Validate::is($password , 'NotEmpty')) {
			$result['error'] = $helper->__('The password cannot be empty.');
		}
		if (strlen($password) && !Zend_Validate::is($password, 'StringLength', array(6))) {
			$result['error'] = $helper->__('The minimum password length is %s', 6);
		}
		if ($password != $passwordConfirmation) {
			$result['error'] = $helper->__('Please make sure your passwords match.');
		}
		...
		//$result['urlRedirect'] = Mage::getUrl('onestepcheckout/onepage/success');
