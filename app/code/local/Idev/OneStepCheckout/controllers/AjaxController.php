...
class Idev_OneStepCheckout_AjaxController extends Mage_Core_Controller_Front_Action
{
    ...
    public function add_couponAction()
    {
		...
		$html = $this->loadLayout()->getLayout()->getBlock('onestepcheckout.summary')->toHtml();
    ...
    public function add_giftcardAction(){
		...
		$html = $this->loadLayout()->getLayout()->getBlock('choose-payment-method')->toHtml();
    ...
    public function save_billingAction()
    {
        ...
        if(Mage::helper('customer')->isLoggedIn() && $helper->differentShippingAvailable()){
            if(!empty($customerAddressId)){
                ...
                if(is_object($billingAddress) && $billingAddress->getCustomerId() ==  Mage::helper('customer')->getCustomer()->getId()){
                    $billing_data = array_merge($billingAddress->getData(), $billing_data);
                }
            }
            if(!empty($shippingAddressId)){
                ...
                if(is_object($shippingAddress) && $shippingAddress->getCustomerId() ==  Mage::helper('customer')->getCustomer()->getId()){
                    $shipping_data = array_merge($shippingAddress->getData(), $shipping_data);
                }
            }
        }
        ...
        try {
            if($paymentMethod){
				...
				$billing_data['telefone'] = $billing_data['telephone'];


				if(Mage::getStoreConfig('onestepcheckout/general/update_address_and_customer')) {
					//Save customer data
					$customer_data = array_merge(Mage::getSingleton('customer/session')->getCustomer()->getData(), $billing_data);
					$customerForm = Mage::getModel('customer/form')
						-> setFormCode('customer_account_create')
						-> setEntity(Mage::getSingleton('customer/session')->getCustomer());

					$customerErrors = $customerForm->validateData($customer_data);

					try {
						$customerForm->compactData($customer_data);
						Mage::getSingleton('customer/session')->getCustomer()->save();
					} catch (Mage_Core_Exception $e) {
						Mage::log('Customer Save Error::' . $e->getMessage());
					} catch (Exception $e) {
						Mage::log('Customer Save Error::' . $e);
					}
				}

				/* 4Vets address save fix */
				if ($billing_data['save_in_address_book'])
				{
