...
class Allpago_Gwap_Model_Methods_2cc extends Mage_Payment_Model_Method_Cc {
    ...
    public function assignData($data) {
        ...
        $info->setCcType($data->getData('gwap_2cc_type'))
                ...
                ->setCcNumber($data->getData('gwap_2cc_number'))
                ->setCcCid($data->getData('gwap_2cc_cid'))
                ...
                ->setAdditionalInformation('gwapCcAmount', str_replace(',','.',str_replace('R$ ','',$data->getData('gwap_2cc_amount'))))
                ...
                ->setAdditionalInformation('gwapCcNumber2', $data->getData('gwap_2cc_number2'))
                ->setAdditionalInformation('gwapCcCid2', $data->getData('gwap_2cc_cid2'))
                ...
                Mage::getModel('core/session')->setGwapCcId($info->getCcCid());
                Mage::getModel('core/session')->setGwapCcCcNumber($info->getCcNumber());                
    ...
    public function authorize(Varien_Object $payment, $amount) {
        ...
        if ($resultCode2 != '90') {
            ...
            $removeResult = $this->removeCreditAuto($payment->getOrder(),$response1['IDENTIFICATION.UNIQUEID'],$cc->getCcType(),1);           
    ...
    public function capture(Varien_Object $payment, $amount) {
        ...
        if ($resultCode2 != '90') {
            ...
            // Em caso de erro, estornar primeiro cartão            
            $removeResult = $this->removeCreditAuto($payment->getOrder(),$response1['IDENTIFICATION.UNIQUEID'],$gwap->getCcType(),1);           
    ...
    public function removeCreditAuto($order,$uniqueId,$ccType,$ccPosition){
        
            $now       = date('d', Mage::getModel('core/date')->timestamp(time()));
            $createdAt = date('d', Mage::getModel('core/date')->timestamp($order->getCreatedAt()));

            if ($now > $createdAt){
                $response = $this->reversalOrRefund($order->getIncrementId(),$uniqueId,'RF',$ccType,$ccPosition);  
            }else{
                $response = $this->reversalOrRefund($order->getIncrementId(),$uniqueId,'RV',$ccType,$ccPosition);  
            }
    ...
    public function _validateFirstCc() {
        ...
        if (in_array($info->getCcType().'_C', $availableTypesC)
            ...
            if ($this->validateCcNum($ccNumber)
                ...
                if ((!$this->OtherCcType($info->getCcType()) && $ccType != $info->getCcType()) && $info->getCcType() != 'CABAL') {
        ...
        if ($errorMsg === false && $this->hasVerification()) {
            $verifcationRegEx = Mage::helper('gwap')->getVerificationRegEx();
    ...
    public function _validateSecondCc() {
        ...
        if (in_array($info->getAdditionalInformation('gwapCcType2').'_C', $availableTypesC)
            ...
            if ($this->validateCcNum($ccNumber2)
                    ...
                    || ($this->OtherCcType($info->getAdditionalInformation('gwapCcType2')) && $this->validateCcNumOther($ccNumber2))) {
                ...
                foreach ($ccTypeRegExpList as $ccTypeMatch => $ccTypeRegExp) {
                    if (preg_match($ccTypeRegExp, $ccNumber2)) {
                        $ccType = $ccTypeMatch;
                        break;
                    }
                }

                if (!$this->OtherCcType($info->getAdditionalInformation('gwapCcType2')) && $ccType != $info->getAdditionalInformation('gwapCcType2')) {
