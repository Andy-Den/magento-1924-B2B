...
class Inchoo_SocialConnect_Helper_Facebook extends Mage_Core_Helper_Abstract
{
    ...
    public function loginByCustomer(Mage_Customer_Model_Customer $customer)
    {
		...
		$customer->setIsAllowedToLogin(1);
        Mage::getSingleton('customer/session')->setCustomerAsLoggedIn($customer);
    }
	...
	public function getPictureUrl($customerId)
	{
		$_picture = null;
		$_customer = Mage::getModel('inchoo_socialconnect/facebook_info_user')->load((int)$customerId);
		if ($_customer->getId())
		{
			$_picture = Mage::helper('inchoo_socialconnect/facebook')
					->getProperDimensionsPictureUrl($_customer->getId(),
					$_customer->getPicture()->data->url);
		}
		return $_picture;
	}
