...
if(Mage::helper('core')->isModuleEnabled('DS_PrivateSales')){
	class WP_CustomMenu_Block_Navigation_Tmp extends DS_PrivateSales_Block_Navigation {}
} else {
	class WP_CustomMenu_Block_Navigation_Tmp extends Mage_Catalog_Block_Navigation {}
}

class WP_CustomMenu_Block_Navigation extends WP_CustomMenu_Block_Navigation_Tmp
{
    ...
    public function drawCustomMenuItem($category, $level = 0, $last = false, $drawPopup = true)
    {
        if (!$category->getIsActive()) return;
        $htmlTop = array();
        $id = $category->getId();
    ...
    private function getCategoryCollection()
    {
        if (!$this->_categoryCollection) {
            $this->_categoryCollection = Mage::helper('custommenu')->getCategoryCollection();
        }
