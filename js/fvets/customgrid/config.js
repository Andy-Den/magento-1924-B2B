...
fvetscg.Tools = {
    ...
    {
        if (!otherWindow && $('fvetscg_window') && (typeof(Windows) != 'undefined')) {
            Windows.focus('fvetscg_window');
            return;
        }
        
        var windowId  = 'fvetscg_window' + (otherWindow ? '_'+(++this.windowsNumber) : ''),
        ...
        windowConfig = Object.extend({
            ...
            closable: true,
            className: 'fvetscg',
            windowClassName: 'popup-window fvetscg-popup-window',
            title: '',
            width: 800,
            height: 450,
            zIndex: 1000,
        ...
        if (windowConfig.resizable) {
            windowConfig.windowClassName += ' fvetscg-resizable-popup-window';
        }
        if (windowUrl) {
...
fvetscg.EventsManager = Class.create();
fvetscg.EventsManager.prototype = {
    initialize: function()
    {
        this.handlers = $H({});
    },
...
fvetscgEventsManager = new fvetscg.EventsManager();
...
fvetscg.Tooltip = Class.create();
fvetscg.Tooltip.prototype = {
    initialize: function(element, tooltip)
    {
        ...
        this.tooltip.addClassName('fvetscg-tooltip');
...
fvetscg.Config = Class.create();
fvetscg.Config.prototype = {
        ...
        if (!rendererOptionsForm.validator
            ...
            new Ajax.Request($(this.formEl).action, {
                ...
                onComplete: function(transport){
                    try {
                        fvetscg.Tools.onAjaxSuccess(transport);
...
if (typeof(fvetscg.Renderer) == 'undefined') {
    fvetscg.Renderer = {};
}

fvetscg.Renderer.Config = Class.create(fvetscg.Config);

fvetscg.Renderer.Select = Class.create();
fvetscg.Renderer.Select.prototype = {
    ...
    {   
        if (this.configButton) {
            ...
            this.configButton.observe('click', function(){ 
                fvetscg.Tools.openDialogFromPost(url, {
                  'code': code,
...
if (typeof(fvetscg.Renderer.Collection) == 'undefined') {
    fvetscg.Renderer.Collection = {};
}
if (typeof(fvetscg.Renderer.Attribute) == 'undefined') {
    fvetscg.Renderer.Attribute = {};
}

fvetscg.Renderer.Collection.Select = Class.create(fvetscg.Renderer.Select);
fvetscg.Renderer.Attribute.Select = Class.create(fvetscg.Renderer.Select, {
...
if (typeof(fvetscg.Form) == 'undefined') {
    fvetscg.Form = {};
}
if (typeof(fvetscg.Form.Element) == 'undefined') {
    fvetscg.Form.Element = {};
}

fvetscg.Form.Element.DependenceController = Class.create();
fvetscg.Form.Element.DependenceController.prototype = {
...
* Copyright (c) Benoît Leulliette <carlos.farah@gmail.com>
* http://opensource.org/licenses/osl-3.0.php  Open Software License (OSL 3.0)
*/

fvetscg.TableDnd = Class.create();
fvetscg.TableDnd.prototype = {
    initialize: function(table, options)
    {
        ...
        fvetscgEventsManager.addUniqueHandler('fvetscg-dnd-'+$(table).identify()+'-mouse-move', 'prototype', document, 'mousemove', onMouseMoveHandler);
        fvetscgEventsManager.addUniqueHandler('fvetscg-dnd-'+$(table).identify()+'-mouse-up', 'prototype', document, 'mouseup', onMouseUpHandler);
    },
...
if (typeof(fvetscg.Grid) == 'undefined') {
    fvetscg.Grid = {};
}

fvetscg.Grid.Export = Class.create();
fvetscg.Grid.Export.prototype = {
    ...
    doExport: function()
    {
        ...
        return fvetscg.Tools.submitContainerValues(this.container, $F(this.formatSelect), this.additional, 'GET');
    }
};

fvetscg.Grid.Config = Class.create();
fvetscg.Grid.Config.prototype = {
    ...
    initElements: function()
    {
        ...
        if (this.config.useDnd) {
            ...
            this.tableDnd  = new fvetscg.TableDnd(this.table, {
                dragHandle: 'fvetscg-drag-handle',
    ...
    saveGrid: function()
    {
        ...
        return fvetscg.Tools.submitContainerValues(this.container, this.saveUrl);
    }
};

fvetscg.Grid.ActionsPinner = Class.create();
fvetscg.Grid.ActionsPinner.prototype = {
    initialize: function(containerId, gridTableId)
    {
        this.container   = $(containerId);
        this.gridTable   = $(gridTableId);
        this.pinnedBlock = this.container.select('.fvetscg-grid-pinnable-actions-block').first();
        
        if (!this.pinnedBlock || this.pinnedBlock.empty()) {
            return;
        }
        
        this.pinnedPlaceholder = this.container.select('.fvetscg-grid-pinned-actions-block-placeholder').first();
        this.isVisible   = true;
        this.isPinned    = false;
        ...
        if (this.pinnedBlock && this.pinnedPlaceholder) {
            ...
            fvetscgEventsManager.addUniqueHandler('fvetscg-gap-'+containerId+'-load',   'prototype', window, 'load',   onScrollHandler);
            fvetscgEventsManager.addUniqueHandler('fvetscg-gap-'+containerId+'-scroll', 'prototype', window, 'scroll', onScrollHandler);
            fvetscgEventsManager.addUniqueHandler('fvetscg-gap-'+containerId+'-resize', 'prototype', window, 'resize', onScrollHandler);
            ...
            fvetscgEventsManager.addUniqueHandler('fvetscg-gap-'+containerId+'-tab-show', 'varien', null, 'showTab', onTabShowHandler);
            fvetscgEventsManager.addUniqueHandler('fvetscg-gap-'+containerId+'-tab-hide', 'varien', null, 'hideTab', onTabHideHandler);
        }
    ...
    pin: function()
    {
        ...
        if (this.pinnedBlock) {
            this.pinnedBlock.select('.fvetscg-additional-filters-table').invoke('addClassName', 'no-display');
            ...
            this.pinnedBlock.addClassName('fvetscg-grid-pinned-actions-block');
    ...
    unPin: function()
    {
        ...
        if (this.pinnedBlock) {
            this.pinnedBlock.select('.fvetscg-additional-filters-table').invoke('removeClassName', 'no-display');
            this.pinnedBlock.removeClassName('fvetscg-grid-pinned-actions-block');
...
fvetscg.Grid.Editor = Class.create();
fvetscg.Grid.Editor.prototype = {
    ...
    {
        return 'fvetscg-column-editor-overlay-' + cell.identify();
    },
    
    createCellOverlay: function(cell)
    {
        var overlay = this.createDiv(this.getCellOverlayId(cell), ['fvetscg-column-editor-overlay']);
    ...
    fillCellOverlay: function(cell, overlay)
    {
        ...
        if (cell.hasClassName('fvetscg-column-editor-editing')) {
            if (!overlay.hasClassName('fvetscg-column-editor-overlay-container-editing')) {
                overlay.innerHTML = '';
                var div = this.createDiv(null, ['fvetscg-column-editor-overlay-validate']);
                div.observe('click', function(){ this.validateEdit(); }.bind(this));
                overlay.appendChild(div);
                div = this.createDiv(null, ['fvetscg-column-editor-overlay-cancel']);
                div.observe('click', function(){ this.cancelEdit(); }.bind(this));
                overlay.appendChild(div);
                overlay.removeClassName('fvetscg-column-editor-overlay-container-idle');
                overlay.addClassName('fvetscg-column-editor-overlay-container-editing');
            }
        } else if (!overlay.hasClassName('fvetscg-column-editor-overlay-container-idle')) {
            overlay.innerHTML = '';
            var div = this.createDiv(null, ['fvetscg-column-editor-overlay-edit']);
            div.observe('click', function(){ this.editCell(cell); }.bind(this));
            overlay.appendChild(div);
            overlay.removeClassName('fvetscg-column-editor-overlay-container-editing');
            overlay.addClassName('fvetscg-column-editor-overlay-container-idle');
        }
    },
    ...
    closeEditWindow: function()
    {
        if (this.editWindow) {
            ...
            fvetscg.Tools.closeDialog(this.editWindow);
            this.editWindow = null;
        }
    },
    ...
    editCell: function(cell)
    {
        ...
        if (!this.compareCells(this.editedCell, cell)) {
            ...
            if (cellConfig.in_grid) {
                        ...
                        try {
                            if (transport.responseText.isJSON()) {
                                ...
                                if (response.error) {
                                    ...
                                    cell.addClassName('fvetscg-column-editor-editing');
                                    editor.previousValue = cell.innerHTML;
                                    editor.hasPreviousValue = true;
                                    
                                    var form = document.createElement('form');
                                    form.id = 'fvetscg-column-editor-form-' + cell.identify();
                                    ...
                                    cell.getElementsBySelector('.fvetscg-editor-required-marker').each(function(e){
                                        e.hide();
                                        cell.addClassName('fvetscg-column-editor-editing-required');
                                    });
                ...
                this.editWindow = fvetscg.Tools.openIframeDialog(editUrl, windowConfig.toObject(), true);
            }
        }
    },
    ...
    validateEdit: function(formParams)
    {
        ...
        if (this.editedCell) {
            ...
            if (cellConfig.in_grid) {
                var form = $('fvetscg-column-editor-form-' + cellId);
                if (form) {
                        ...
                        try {
                            if (transport.responseText.isJSON()) {
                                ...
                                if (response.error) {
                                    ...
                                    var cell = editor.editedCell;
                                    cell.addClassName('fvetscg-column-editor-updated');
                                    cell.removeClassName('fvetscg-column-editor-editing');
                                    cell.removeClassName('fvetscg-column-editor-editing-required');
    ...
    cancelEdit: function(fromDialog, errorMessage)
    {
        ...
        if (this.editedCell) {
            ...
            this.previousValue = null;
            this.editedCell.removeClassName('fvetscg-column-editor-editing');
            this.editedCell.removeClassName('fvetscg-column-editor-editing-required');
...
if (typeof(fvetscg.CustomColumn) == 'undefined') {
    fvetscg.CustomColumn = {};
}

fvetscg.CustomColumn.List = Class.create();
fvetscg.CustomColumn.List.prototype = {
    initialize: function(wrapperId, config)
    {
        this.wrapper = $(wrapperId);
        
        this.config = Object.extend({
            groupClass: 'fvetscg-custom-columns-group',
            labelClass: 'fvetscg-custom-columns-group-label',
            labelOnClass: 'fvetscg-custom-columns-group-label-on',
            labelOffClass: 'fvetscg-custom-columns-group-label-off',
            listClass: 'fvetscg-custom-columns-list',
            buttonClass: 'fvetscg-custom-columns-column-button'
        }, config || {});
    ...
    initButtonTooltips: function()
    {
        this.wrapper.getElementsBySelector('.'+this.config.buttonClass).each(function(button){
            ...
            if (content) {
                new fvetscg.Tooltip(button, content);
            }
        }.bind(this));
    }
} 

fvetscg.MessagesTabs = Class.create();
fvetscg.MessagesTabs.prototype = {
    initialize: function(wrapperId, types, config)
    {
        ...
        this.config = Object.extend({
            tabIdPrefix: 'fvetscg-messages-tab-',
            countIdPrefix: 'fvetscg-messages-tab-count-',
            wrapperIdPrefix: 'fvetscg-messages-list-wrapper-',
            tabClass: 'fvetscg-messages-tab',
            wrapperClass: 'fvetscg-messages-list-wrapper',
...
fvetscg.CustomColumn.Form = Class.create(fvetscg.Config);

fvetscg.CustomColumn.Config = Class.create();
fvetscg.CustomColumn.Config.prototype = {
    initialize: function(code, configButtonId, rendererTargetId, configUrl, windowConfig)
    {
        $(configButtonId).observe('click', function(){ 
            fvetscg.Tools.openDialogFromPost(configUrl, {
              'code': code,
...
fvetscg.CustomColumn.OptionsColor = Class.create();
fvetscg.CustomColumn.OptionsColor.registerRowChange = function(childId, backgroundColor, textColor, onlyCell) {
    var elm    = $(childId),
        upElm  = null,
...
if (typeof(fvetscg.Filter) == 'undefined') {
    fvetscg.Filter = {};
}

fvetscg.Filter.Categories = Class.create();
fvetscg.Filter.Categories.prototype = {
    ...
    openChooser: function()
    {
        ...
        this.window = fvetscg.Tools.openIframeDialog(chooserUrl, this.windowConfig, true);
    },
    ...
    applyChoice: function(ids)
    {
        if (this.window) {
            ...
            fvetscg.Tools.closeDialog(this.window);
        }
    }
};
