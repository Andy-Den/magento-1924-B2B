function wpShowMenuPopup(objMenu, event, popupId)
{
    ...
    if (!objMenu.hasClassName('active')) {
        ...
        if (jQuery != undefined)
        {
       /* jQuery('#'+popupId).find('.itemSubMenu .itemMenu').each(function() {
            jQuery(this).css({'minHeight' : jQuery('#'+popupId).height() - 24 + 'px'});
            console.log(jQuery(this).children('.itemMenuColumn2'));
        })*/

            $(popupId).select('.itemSubMenu').each(function(item, index) {
                item.style.marginLeft = $(popupId).getWidth() + 'px';
            });

            $(popupId).select('.itemSubMenu .itemMenu').each(function(item, index) {
                item.style.minHeight = $(popupId).getHeight() - 24 + 'px';


                if (item.select('.itemMenuColumn2').length > 0)
                {
                    item.style.width = 350+'px';
                }
            });
        }
    }
...
function wpCustomMenuMobileToggle()
{
    ...
    //if ((x < 100 || wpIsMobile.any()) && wpMobileMenuEnabled) {
    if ((x < 768 || wpIsMobile.any()) && wpMobileMenuEnabled) {
        $('custommenu').hide();
