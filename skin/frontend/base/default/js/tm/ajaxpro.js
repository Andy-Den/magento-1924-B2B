...
AjaxPro = function () {
    ...
    return {
        ...
        request: function (request) {
            ...
            request.parameters = Object.extend(
                {'handles[]': _getConfig('handles'), ajaxpro: 1, in_cart: 1, 'options[]':request.options},
                request.parameters || {}
            );
            ...
            new Ajax.Request(request.url, {
                ...
                onComplete: function(transport) {
                    ...
                    //exibe a mensagem de erro (caso haja) para o usuário
                    if (response.errorMsg != undefined) {
                        //$$('.page')[0].insert('<div id="divAjaxError" class="divAjaxError"><span id="msgAjaxError" class="msgAjaxError"></span></div>');
                        $('msgAjaxError').update(response.errorMsg);
                        jQuery( "#divAjaxError" ).slideUp( 300 ).delay( 800 ).fadeIn( 400 );
                        jQuery("#divAjaxError").delay(7000).fadeOut('slow');
                        //setando true no status para que não seja redirecionado
                        response.status = true;
                    }

                    //renderizando ou escondendo o botão de checkout dependendo da quantidade de itens no carrinho;
                    if (response.hasOwnProperty('cartItemsCount')) {
                        var checkoutButtonEl = jQuery('#checkoutButton');
                        if(response.cartItemsCount > 0) {
                            checkoutButtonEl.show();
                        } else {
                            checkoutButtonEl.hide();
                        }
                    }
    ...
    //                }
                    ...
                    if (transport.request.parameters.methodToExecute) {
                        try {
                            eval(transport.request.parameters.methodToExecute);
                        } catch (err) {
                            //let the script continues
                        }
                    }
                }
            });
