...
var OneStepCheckoutLoginPopup = Class.create({
    initialize: function(options) {
        ...
        this.inscricao_estadual_isento = $('billing:inscricao_estadual_isento');
        this.inscricao_estadual = $('billing:inscricao_estadual');
    ...
    bindEventHandlers: function() {
        ...
        /* */
        if (this.inscricao_estadual_isento) {
            this.inscricao_estadual_isento.on('change', function(event, element){
                if(element.checked === true) {
                    this.inscricao_estadual.disable();
                    this.inscricao_estadual.removeClassName('required-entry');
                } else {
                    this.inscricao_estadual.enable();
                    this.inscricao_estadual.addClassName('required-entry');
                }
            }.bind(this));
        }
    },
...
function get_totals_element(salesrep)
{
    // Search for OSC summary element
    if (salesrep)
        var search_osc = $$('div.onestepcheckout-summary-'+salesrep);
    else
    ...
    if (salesrep)
        var search_cart = $$('div.shopping-cart-totals-'+salesrep);
    else
    ...
    if (salesrep)
        if($('shopping-cart-totals-table-'+salesrep))    {
            return $('shopping-cart-totals-table-'+salesrep);
        }
    else
...
function get_save_methods_function(url, update_payments, salesrep)
{
    ...
    return function(e)    {
        ...
        totals.update('<div class="loading-ajax fa fa-spinner fa-spin">&nbsp;</div>');

        if(update_payments)    {
            var payment_methods = $$('div.payment-methods-'+salesrep)[0];
            payment_methods.update('<div class="loading-ajax fa fa-spinner fa-spin">&nbsp;</div>');
        }
        ...
        var items = $$('input[name^=payment\['+salesrep+'\]]').concat($$('select[name^=payment\['+salesrep+'\]]'));
        var names = items.pluck('name');
        var values = items.pluck('value');

        for(var x=0; x < names.length; x++)    {
            if(names[x] != 'payment['+salesrep+'][method]')    {
            ...
            onSuccess: function(transport)    {
                if(transport.status == 200)    {
                    ...
                    if(update_payments)    {
                        ...
                        $$('div.payment-methods input[name="payment\['+salesrep+'\]\[method\]"]').invoke('observe', 'click', get_separate_save_methods_function(url));

                        $$('div.payment-methods input[name="payment\['+salesrep+'\]\[method\]"]').invoke('observe', 'click', function() {
...
function get_save_billing_function(url, set_methods_url, update_payments, triggered)
{
    ...
    return function()    {
        ...
        /*if(shipment_methods_found)  {
            shipment_methods.update('<div class="loading-ajax fa fa-spinner fa-spin">&nbsp;</div>');
        }*/
        ...
        //$('billing_address_list').update('<div class="loading-ajax">&nbsp;</div>');
        ...
        new Ajax.Request(url, {
            ...
            onSuccess: function(transport)    {
                if(transport.status == 200)    {
                    ...
                    $('billing_address_list').update(data.billing_address_list);
            ...
            onComplete: function(transport){
                ...
                if(transport.status == 200)    {
                    ...
                    if(update_payments){
                        $$('div.payment-methods input[name="payment\['+salesrep+'\]\[method\]"]').invoke('observe', 'click', get_separate_save_methods_function(set_methods_url));
...
function get_separate_save_methods_function(url, update_payments, salesrep)
{
    if(typeof update_payments == 'undefined')    {
        var update_payments = false;
    }
    ...
    return function(e)    {
        ...
        var shipping_method = $RF(form, 'shipping_method');
        var payment_method = $RF(form, 'payment['+salesrep+'][method]');
        var summary = get_totals_element(salesrep);
        var totals_final = get_totals_element(false);

        var freeMethod = $('p_method_free-'+salesrep);
        if(freeMethod){
        ...
        if(summary !== undefined) {
            summary.update('<div class="loading-ajax fa fa-spinner fa-spin fa-2x">&nbsp;</div>');
        }
        if(totals_final !== undefined) {
            totals_final.update('<div class="loading-ajax fa fa-spinner fa-spin fa-2x">&nbsp;</div>');
        }

        if(update_payments)    {
            var payment_methods = $$('div.payment-methods-'+salesrep)[0];
            payment_methods.update('<div class="loading-ajax fa fa-spinner fa-spin fa-2x">&nbsp;</div>');
        }
        ...
        new Ajax.Request(url, {
            ...
            onSuccess: function(transport)    {
                if(transport.status == 200)    {
                    ...
                    totals_final.update(data.totals);

                    if(update_payments)    {

                        payment_methods.replace(data.payment_method);

                        $$('div.payment-methods input[name="payment\['+salesrep+'\]\[method\]"]').invoke('observe', 'click', get_separate_save_methods_function(url, null, salesrep));
                        $$('div.payment-methods input[name="payment\['+salesrep+'\]\[method\]"]').invoke('observe', 'click', function() {
                            $$('div.onestepcheckout-payment-method-error').each(function(item) {
                                new Effect.Fade(item);
                            });
                        ...
                        if($RF($('onestepcheckout-form'), 'payment['+salesrep+'][method]') != null)    {
                            try    {
                                var payment_method = $RF(form, 'payment['+salesrep+'][method]');
                                $('container_payment_method_' + payment_method).show();
                                $('payment_form_' + payment_method).show();
...
function paymentrefresh(url, salesrep) {
    var payment_methods = $$('div.payment-methods-'+salesrep)[0];
    payment_methods.update('<div class="loading-ajax fa fa-spinner fa-spin">&nbsp;</div>');
    new Ajax.Request(url, {
        method: 'get',
        onSuccess: function(transport){
            if(transport.status == 200)    {
                ...
                $$('div.payment-methods input[name="payment\['+salesrep+'\]\[method\]"]', 'div.payment-methods input[name="payment\[useProfile\]"]').invoke('observe', 'click', get_separate_save_methods_function(url));
                $$('div.payment-methods input[name="payment\['+salesrep+'\]\[method\]"]', 'div.payment-methods input[name="payment\[useProfile\]"]').invoke('observe', 'click', function() {
                    $$('div.onestepcheckout-payment-method-error').each(function(item) {
                        new Effect.Fade(item);
                    });
                });
...
function paymentrefresh2(url, salesrep) {
    if(typeof e != 'undefined')    {
        var element = e.element();
    ...
    var payment_methods_container = $$('div.payment-methods-'+salesrep)[0];
    //payment_methods.update('<div class="loading-ajax fa fa-spinner fa-spin">&nbsp;</div>');
    jQuery('.loading-overlay').show();
    new Ajax.Request(url, {
        method: 'get',
        onSuccess: function(transport){
            ...
            if(transport.status == 200)    {
                ...
                payment_methods_container.replace(data);

                // $$('div.payment-methods-'+salesrep+' input[name="payment\['+salesrep+'\]\[method\]"]').each(function(h) {
                //     $(h).observe('click', paymentrefresh2(url, salesrep));
                // });
        ...
        onComplete: function() {
            ...
            checkoutShowNextFrame(salesrep);
        },
        parameters: parameters
    });
}
...
Payment.prototype = {
    ...
    init : function () {
        ...
        for (var i=0; i<elements.length; i++) {
            if (elements[i].name=='payment['+salesrep+'][method]') {
    ...
    validate: function() {
        var methods = document.getElementsByName('payment['+salesrep+'][method]');
        if (methods.length==0) {
