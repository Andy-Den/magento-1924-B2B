...
function wpInitPopupContent()
{
    ...
    //corrigindo altura do menu perante a resoluçao do cliente
    fixMenuHeightMedia();
}
...
function fixMenuHeightMedia() {
    var $element = jQuery('.header-container');
    var height = jQuery(window).height() - $element.height() - 100;
    //alert(jQuery(window).height());
    //alert($element.offset().top);
    jQuery('.wp-custom-menu-popup').css('max-height', height+'px');
    jQuery('.wp-custom-menu-popup').css('overflow', 'auto');
}