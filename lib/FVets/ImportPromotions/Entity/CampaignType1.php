...
class Entity_CampaignType1 extends Abstracts_Campaign implements Interfaces_ICampaign
{
	protected $promotionType = 1;
	protected $defaultStopRulesProcessing = 1;
	protected $defaultLabelShortName = 'Promoção';
	protected $defaultLabelDescription = 'Compre acima de %s unidades e ganhe %s de desconto';
	protected $requiredFields = array('from_date', 'to_date', 'ids_erp', 'min_qty', 'discount_ammount');
	protected $discountType = 'by_percent';

	function _initName()
	{
		$idsErpName = $this->getFunctions()->getIdsErpForRuleName($this->getIdsErp());
		$name = $this->getWebsiteCode() . "_type_" . $this->promotionType . "_idserp_" . $idsErpName . "_minqty_" . $this->getMinQty() . '_percent_' . $this->getDiscountAmmount();
		return $name;
	}
	...
	function _initDescription()
	{
		if (substr_count($this->getLabelDescription(), '%s') == 2) {
			$description = $this->getHelper()->__($this->getLabelDescription(), $this->getMinQty(), $this->getDiscountAmmount() . '%');
		} else {
