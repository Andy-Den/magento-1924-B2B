...
if (empty($lines)){
    ...
    foreach ($lines as $key => $value) {
        ...
        $tipo = $temp[0];
        $idErp = trim($temp[1]);
        if ((!empty($temp[21]))): $lastOrderErp = date('Y-m-d', strtotime($temp[21])); else: $lastOrderErp = NULL; endif;

        if (is_null($lastOrderErp)) {
            echo "\n\n O cliente com o ID ERP: $idErp Não possue data da ultima compra definida \n\n";
        ...
        if($tipo != 3){
            ...
            if ($entityId == true) {
                ...
                if (($lastOrderErpAtual == false) && ($lastOrderErpAtual != '')) {
                ...
                if ($origemAtual != 'SITE'){
                    if ($origemAtual == false) {
                        ...
                        echo "\n\n A origem do cliente com o ID ERP: $idErp foi definido como: $origem\n\n";
                    ...
                    if ($origemAtual != $origem) {
                        ...
                        echo "\n\n A origem do cliente com o ID ERP: $idErp foi atualizado para: $origem\n\n";
                ...
                echo "\n\n Cliente não localizado pelo ID ERP: $idErp\n\n";
    ...
    if (!is_null($origemDefinido)) {
        ...
        echo "\n Arquivo com Relação de clientes e a sua origem foi criado.\n";
    }
    ...
    if (!is_null($idErpNaoLocalizado)) {
        ...
        echo "\n Arquivo com Relação de clientes não localizados em nossa base de dados foi criado.\n";
    }
    // Gera o arquivo com clientes não localizados em nossa base de dados
    if (!is_null($semData)) {
        file_put_contents("$directoryRep/$currentDate/clienteSemData.csv", $semData);
        echo "\n Arquivo com Relação de clientes data da ultima compra foi criado.\n";
    }
}