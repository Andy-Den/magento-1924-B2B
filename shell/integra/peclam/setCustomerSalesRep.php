...
if ($local == true ):$lines = file("$testDirectoryImport/repsCustomers.csv", FILE_IGNORE_NEW_LINES);
else : $lines = file("$directoryImport/repsCustomers.csv", FILE_IGNORE_NEW_LINES); endif;
...
if (empty($lines)){
    ...
    foreach ($lines as $key => $value) {
        ...
        if ($rep1 != NULL){
            ...
            $getIdRep1 .= " WHERE store_id IN ($storeId) AND ";
