...
} else
{
	try
	{
		...
		foreach ($lines as $key => $value)
		{
			...
			if ($dataInicio) {
				$format = 'd/m/Y H:i:s';
				$date = date_create_from_format($format, $dataInicio);
				if ($currentDate < $date) {
					continue;
				}
			} else {
			...
			if ($dataFim) {
				$format = 'd/m/Y H:i:s';
				$date = date_create_from_format($format, $dataFim);
				if ($currentDate > $date) {
 					continue;
				}
			} else {
		...
		addPromotions($promotions, $storeviewId, $websiteId, $resource, $readConnection, $writeConnection);

		Mage::helper('datavalidate')->sendSlackMessage($channel, "[Campanhas - Fim Integração] Integração ERP da $codeStore finalizada");

	} catch (Exception $ex)
	{
		Mage::helper('datavalidate')->sendSlackMessage($errorChannel, "[Campanhas - Erro na integracao ERP da $codeStore] Erro: (" . $ex->__toString() . ")");
	}
}
...
function addPromotions($promotions, $storeviewId, $websiteId)
{
	...
	foreach ($promotions as $key => $promotion)
	{
		...
		} else
		{
			...
			$idsErp = explode(',', $key);
			unset($idsErp[0]);
			unset($idsErp[count($idsErp)]);
			$idsErp = implode(',', $idsErp);
			...
			$qtdMinima = substr($key, strrpos($key, ',') + 1);
		...
		} elseif ($tipoPromocao == 3 || $tipoPromocao == 4)
		{
			...
			{
				case 3:
					$nameRule = "omega" . "_" . $tiposPromoArray[$tipoPromocao] . "_tipo_" . $tipoPromocao . "_idserp_" . str_replace(',', '-', $idsErp) . "_qtdmin_" . $qtdMinima . '_leve_' . ($qtdMinima + $qtdProdutoBonificado) . '_unidades_do_produto_' . $produtosBonificados;
					break;
				case 4:
			...
			$values = "VALUES ('$nameRule', '$description', STR_TO_DATE('$dataInicio','%d/%m/%Y'), STR_TO_DATE('$dataFim','%d/%m/%Y'), 0, 1, $condition, $action, $stopRulesProcessing, 1, NULL, $priority, 'ampromo_items', $qtdProdutoBonificado, NULL, $qtdMinima, 0, 0, 0, 1, 1, 0, 0, '$skuProdutoBonificado');";
		} else
		{
			continue;
		}
		...
		$getRuleId .= " ORDER BY rule_id DESC LIMIT 1;";
