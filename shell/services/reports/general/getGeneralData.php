...
foreach ($websites as $website) {
	...
	foreach ($stores as $store) {
		$arrayTmp = array("store_code" => $store->getCode(), "store_name" => (count($stores) > 1 ? "[" . $website->getName() . "] " . $store->getName() : $store->getName()), "total_products" => getTotalProducts($store), "active_visible_products" => getVisibleProducts($store), "total_customers" => getTotalCostumers($store));
